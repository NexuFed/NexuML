...
- Add a dedicated GitHub Actions workflow that uses `ubuntu-latest` to build and verify the Linux AMD64 CUDA image on trusted `main`, semantic release-tag, and manual runs.
...
- Automation: a new `.github/workflows/docker.yml` workflow running on `ubuntu-latest`, with GHCR package permissions and dormant Docker Hub configuration comments.
...
- Operations: the GitHub-hosted runner provides Docker with Buildx and must have enough disk space for the CUDA development image.
